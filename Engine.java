...
public class Engine {
    ...
    /**
     * Constructor for the Engine class. Initializes the engine with a specified
     * fuel type and maximum fuel level. The engine starts with a full tank of
     * fuel.
     *
    ...
    /**
     * Consumes fuel as the engine runs. If there is sufficient fuel, it reduces
     * the fuel level by 10 units.
     *
     * @return true if the engine successfully consumes fuel, false if fuel is
     * empty.
     */
    public boolean go() {
        if (this.currentFuelLevel > 0) {
            double fuelConsumed = Math.min(10, currentFuelLevel); // Consume up to 10 units
            currentFuelLevel -= fuelConsumed;
            System.out.println("Fuel consumed: " + fuelConsumed + " units. Remaining fuel: " + currentFuelLevel + " units.");
            return true;
        } else {
            System.out.println("Fuel level is empty. The engine cannot run.");
            return false;
        }
    }
